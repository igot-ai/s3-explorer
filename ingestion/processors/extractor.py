...
class ArchiveExtractor(FileProcessor):
    ...
    SUPPORTED_FORMATS = {
        ...
        'bz2', 'bz3', 'cab', 'chm', 'Z', 'cpio', 'deb', 'dms',
        'flac', 'gz', 'iso', 'lrz', 'lha', 'lzh', 'lz', 'lzma',
        'lzo', 'rpm', 'rar', 'rz', 'shn', 'tar', 'udf', 'xz',
        'zip', 'jar', 'zoo', 'zst',
        # Compound formats
        'tar.gz', 'tgz', 'tar.bz2', 'tbz2', 'tar.xz', 'txz',
        'tar.lz', 'tar.lzma', 'tar.zst'
    }
    ...
    def process(self, file_context: FileContext, output_dir: Path) -> List[FileContext]:
        ...
        try:
            ...
            logger.info(f"Extracted {len(extracted_files)} files from {file_context.source_path}")
...
class SimpleZipExtractor(FileProcessor):
    ...
    def can_process(self, file_context: FileContext) -> bool:
        ...
        Returns:
            True if file is ZIP
        """
        return file_context.file_type == 'zip' or file_context.source_path.lower().endswith('.zip')
