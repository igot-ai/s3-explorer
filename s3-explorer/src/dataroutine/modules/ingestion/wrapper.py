...
if TYPE_CHECKING:
    from dataroutine.modules.ingestion.core.interfaces import IngestionClientProtocol
...
def init_ingestion_wrapper(ingestion_client: "IngestionClientProtocol") -> None:
    ...
    if _wrapper_instance is not None:
        logger.warning(
            "Ingestion wrapper already initialized, skipping re-initialization"
        )
        return
...
def run_ingestion_pipeline(**kwargs) -> "IngestionTaskResult":
    ...
    """
    from dataroutine.shared.temporal_client import run_async
