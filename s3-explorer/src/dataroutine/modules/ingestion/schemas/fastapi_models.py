...
class CatalogModel(BaseModel):
    ...
    metadata_scan: dict = Field(
        default_factory=dict, description="Static metadata prompts for extraction"
    )
...
class IngestionConfigModel(BaseModel):
    ...
    temp_dir: Optional[str] = Field(
        default=None,
        description="Temporary directory (uses env TEMP_DIR if not provided)",
    )
    reader_type: Optional[str] = Field(
        default=None,
        description="Document reader type (markitdown, pdfplumber, pymupdf)",
    )
    ...
    workspace_id: Optional[str] = Field(
        default=None, description="Workspace ID for API handler (required)"
    )
    ...
    user_id: Optional[str] = Field(
        default=None, description="Optional user ID who initiated the job"
    )
...
class IngestionRequestModel(BaseModel):
    ...
    catalogs: List[CatalogModel] = Field(
...
class ListPrefixesRequestModel(BaseModel):
    ...
    storage_credentials: StorageCredentialsModel = Field(
