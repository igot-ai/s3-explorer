...
class IngestionPipelineWorkflow:
    ...
    async def run(self, params: IngestionJobParams) -> Dict[str, Any]:
        ...
        workflow.logger.info(
            f"🚀 Starting IngestionPipelineWorkflow (task_id={params.task_id})"
        )
        ...
        workflow.logger.info(
            f"📊 Pipeline result: success={result.get('success')}, files={result.get('total_files')}"
        )
        workflow.logger.info(
            f"✅ IngestionPipelineWorkflow completed (task_id={params.task_id})"
        )
        return result
