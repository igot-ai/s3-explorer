...
class StorageCredentialsSchema(BaseModel):
    ...
    access_key: Optional[str] = Field(
        default=None, description="Access key for storage provider"
    )
    secret_key: Optional[str] = Field(
        default=None, description="Secret key for storage provider"
    )
    bucket: Optional[str] = Field(default=None, description="Bucket name")
    region: Optional[str] = Field(
        default="us-east-1", description="Region for storage provider"
    )
    account_id: Optional[str] = Field(
        default=None, description="Account ID (for Cloudflare)"
    )
    project_id: Optional[str] = Field(default=None, description="Project ID (for GCS)")
    bucket_name: Optional[str] = Field(
        default=None, description="Bucket name (for GCS/Backblaze)"
    )
    credentials_json: Optional[str] = Field(
        default=None, description="Credentials JSON (for GCS)"
    )
    application_key_id: Optional[str] = Field(
        default=None, description="Application Key ID (for Backblaze)"
    )
    application_key: Optional[str] = Field(
        default=None, description="Application Key (for Backblaze)"
    )
...
class IngestionConfigSchema(BaseModel):
    ...
    classifier_type: str = Field(
        default="openai",
        description="Classifier type (openai, anthropic, ollama, azure)",
    )
    classifier_model: Optional[str] = Field(
        default=None, description="Classifier model name"
    )
    classifier_api_key: Optional[str] = Field(
        default=None, description="Classifier API key"
    )
    classifier_api_base_url: Optional[str] = Field(
        default=None, description="Classifier API base URL"
    )
    classifier_api_version: Optional[str] = Field(
        default=None, description="Classifier API version"
    )

    # Handler settings
    handler_type: str = Field(default="api", description="Handler type (s3, api)")
    api_base_url: Optional[str] = Field(
        default=None, description="API base URL for collection handler"
    )
    api_key: Optional[str] = Field(
        default=None, description="API key for collection handler"
    )

    # Reader settings
    reader_type: str = Field(
        default="pymupdf", description="Document reader type (pymupdf, pdfplumber)"
    )

    # Storage provider settings
    storage_provider: str = Field(
        default="aws", description="Storage provider type (aws, gcs, cloudflare, etc.)"
    )
    storage_credentials: StorageCredentialsSchema = Field(
        default_factory=StorageCredentialsSchema
    )

    # API Handler settings
    workspace_id: Optional[str] = Field(
        default=None, description="Workspace ID for API handler"
    )
    project_id: Optional[str] = Field(
        default=None, description="Project ID for API handler"
    )
    auth_token: Optional[str] = Field(
        default=None, description="Auth token for API handler"
    )
...
class IngestionRequestSchema(BaseModel):
    ...
    catalogs: List[CatalogSchema] = Field(
        ..., min_length=1, description="List of catalogs for classification"
    )
