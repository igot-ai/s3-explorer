...
class StorageCredentialsSchema(Schema):
    ...
    access_key = fields.Str(
        load_default=None, metadata={"description": "Access key for storage provider"}
    )
    secret_key = fields.Str(
        load_default=None, metadata={"description": "Secret key for storage provider"}
    )
    bucket = fields.Str(load_default=None, metadata={"description": "Bucket name"})
    region = fields.Str(
        load_default="us-east-1",
        metadata={"description": "Region for storage provider"},
    )
    account_id = fields.Str(
        load_default=None, metadata={"description": "Account ID (for Cloudflare)"}
    )
    project_id = fields.Str(
        load_default=None, metadata={"description": "Project ID (for GCS)"}
    )
    bucket_name = fields.Str(
        load_default=None, metadata={"description": "Bucket name (for GCS/Backblaze)"}
    )
    credentials_json = fields.Str(
        load_default=None, metadata={"description": "Credentials JSON (for GCS)"}
    )
    application_key_id = fields.Str(
        load_default=None,
        metadata={"description": "Application Key ID (for Backblaze)"},
    )
    application_key = fields.Str(
        load_default=None, metadata={"description": "Application Key (for Backblaze)"}
    )
...
class IngestionConfigSchema(Schema):
    ...
    # Processing settings
    pages_to_read = fields.Int(
        load_default=3,
        validate=validate.Range(min=1),
        metadata={"description": "Number of pages to read from each document"},
    )
    temp_dir = fields.Str(
        load_default=None,
        metadata={
    ...
    storage_provider = fields.Str(
        load_default="aws",
        metadata={"description": "Storage provider type (aws, gcs, cloudflare, etc.)"},
    )
...
class IngestionRequestSchema(Schema):
    ...
    config = fields.Nested(
        ...
        required=True,
        metadata={"description": "Pipeline configuration"},
    )
