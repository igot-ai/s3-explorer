...
def _build_job_config(params: IngestionJobParams) -> IngestionJobConfig:
    ...
    catalogs = [Catalog(id=cat["id"], instruction=cat["instruction"], fetch_all_metadata=cat.get("fetch_all_metadata", False)) for cat in params.catalogs]
    return IngestionJobConfig(source_path=source_path, catalogs=catalogs, recursive=params.recursive, temp_dir=params.temp_dir, pages_to_read=params.pages_to_read)
...
def _build_result(pipeline_result, task_id: str) -> IngestionResult:
    ...
    for folder_ctx in pipeline_result.folder_contexts:
        files = [
            {"source_path": f.source_path, "status": f.status.value, "classified_catalog_id": f.classified_catalog_id, "error_message": f.error_message, "metadata": f.metadata}
